...
(function () {
  ...
  function renderTask() {
    ...
    // Botão de deletar tarefa (se for owner)
    if (isOwner) {
      ...
      if (!document.getElementById('delete-task-btn')) {
        ...
        deleteBtn.className = 'ml-auto text-red-500 hover:text-red-700 p-2 rounded-full hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors';
        deleteBtn.innerHTML = '<span class="material-icons-outlined">delete</span>';
        deleteBtn.onclick = (e) => {
          e.stopPropagation();
          window.deleteTaskCurrent();
        };

        // Inserir antes do checkbox ou append no final
        taskCard.appendChild(deleteBtn);
      }
    }
  }

  // Deletar tarefa atual
  window.deleteTaskCurrent = async function () {
    ...
    try {
      ...
      setTimeout(() => window.location.href = `/list.html?id=${listId}`, 1000);
    } catch (error) {
